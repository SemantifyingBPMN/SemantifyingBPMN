...
extends DEMOPattern{
	...
	public Lane CreateElements_and_Sequence(Lane lane , TransactionKind tk, ArrayList<BPMNMessageFlow> MessageFlows , ArrayList<String> deps , PatternView view) {
	    ...
	    {
		    updateMessageFlow(MessageFlows, tk, bpmn_elements[0], "request (C-act)");
		    updateMessageFlow(MessageFlows, tk, bpmn_elements[10], "accept (C-act)");
		    updateMessageFlow(MessageFlows, tk, bpmn_elements[2] ,"promise (C-act)");
		    updateMessageFlow(MessageFlows, tk, bpmn_elements[8] ,"declare (C-act)");
	    }
	    else 
	    {
			MessageFlows.add(new BPMNMessageFlow(tk, bpmn_elements[0], "request (C-act)", false) );
			MessageFlows.add(new BPMNMessageFlow(tk, bpmn_elements[10], "accept (C-act)",  false) );
			MessageFlows.add(new BPMNMessageFlow(tk, bpmn_elements[2] ,"promise (C-act)", true) );
			MessageFlows.add(new BPMNMessageFlow(tk, bpmn_elements[8] ,"declare (C-act)", true) );	    	
	    }
